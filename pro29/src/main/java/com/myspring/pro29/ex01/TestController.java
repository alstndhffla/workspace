...
public class TestController {
  ...
  @RequestMapping(value= "/notice/{num}" , method = RequestMethod.GET)	//브라우저에서 요청시 {num} 부분의 값이 @PathVariable로 지정.
  public int notice(@PathVariable("num") int num ) throws Exception {	//요청 URL에서 지정된 값이 num에 자동으로 할당
	  return num;
  }	

  @RequestMapping(value= "/info", method = RequestMethod.POST)
  public void modify(@RequestBody MemberVO vo ){	//JSON으로 전송된 데이터를 MemberVO 객체의 속성에 자동으로 설정해 준다.
  ...
  public  ResponseEntity<List<MemberVO>> listMembers2() {	//ResponseEntity로 응답
	...
	public ResponseEntity res3() {
		...
		//ResponseEntity 를 이용해 HTML 형식으로 전송
