...
public class BoardController {
	...
	@RequestMapping(value = "/all", method = RequestMethod.GET)	//GET방식으로 요청하므로 모든 글의 정보를 조회
	...
	@RequestMapping(value = "/{articleNO}", method = RequestMethod.GET)	//GET 방식으로 요청하면서 글 번호를 전달하므로 글 번호에 대한 글 정보를 조회
	...
	@RequestMapping(value = "", method = RequestMethod.POST)	//POST방식으로 요청하므로 요청시 JSON으로 전달되는 객체를 새 글로 추가
	...
	//수정하기
	@RequestMapping(value = "/{articleNO}", method = RequestMethod.PUT)	//PUT 방식으로 요청하므로 articleNO에 대한 글을 전달되는 JSON 정보로 수정
	public ResponseEntity<String> modArticle (@PathVariable("articleNO") Integer articleNO, @RequestBody ArticleVO articleVO) {	//전송된 JSON 회원 정보를 바로 ArticleVO 객체의 속성에 설정
		ResponseEntity<String>  resEntity = null;
		try {
	...
	//삭제하기
	@RequestMapping(value = "/{articleNO}", method = RequestMethod.DELETE)	//DELETE 방식으로 요청하므로 전달되는 articleNO에 대한 글을 삭제
